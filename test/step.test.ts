...
describe('step v0', () => {
  it('accelerates toward gaitSpeed[gait] at gaitAccel', () => {
    ...
    step(s, frame({ gait: 2, throttle: 0 }), r, [], tuning, rng);
  ...
  it('decelerates at gaitDecel and never overshoots', () => {
    ...
    for (let i = 0; i < 600; i++) step(s, frame({ gait: 4, throttle: 0 }), r, [], tuning, rng);
    expect(s.speed).toBeCloseTo(tuning.gaitSpeed[4]!, 9);
    let prev = s.speed;
    let reached = false;
    for (let i = 0; i < 200; i++) {
      step(s, frame({ gait: 1, throttle: 0 }), r, [], tuning, rng);
  ...
  it('brace decelerates to braceSpeed', () => {
    ...
    for (let i = 0; i < 600; i++) step(s, frame({ gait: 3, throttle: 0 }), r, [], tuning, rng);
    expect(s.speed).toBeCloseTo(tuning.gaitSpeed[3]!, 9);
    step(s, frame({ gait: 3, throttle: 0, brace: true }), r, [], tuning, rng);
    expect(s.braced).toBe(true);
    for (let i = 0; i < 200; i++) step(s, frame({ gait: 3, throttle: 0, brace: true }), r, [], tuning, rng);
  ...
  it('gait 2 arrives with (1 - reserveBudget) of the reserve left on any length', () => {
    for (const len of [400, 960]) {
      ...
      for (let i = 0; i < 20000 && !s.ended; i++) step(s, frame({ gait: 2, throttle: 0 }), r, [], tuning, rng);
  ...
  it('ends with stalled when reserve hits 0 (gait 1 cannot finish)', () => {
    ...
    for (let i = 0; i < 20000 && !s.ended; i++) step(s, frame({ gait: 1, throttle: 0 }), r, [], tuning, rng);
