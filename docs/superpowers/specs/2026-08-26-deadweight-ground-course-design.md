...
- **Bot** (`src/sim/bot.ts`): `throttle = 1`, never jumps (solvability must not depend on jumping). Lane planner: for the next fork within 60 m score each lane = Σ impulse of hazards in it (movers ×1.5) + `mud ? 0.6 : 0` + `chicane ? 10 + baffles·0.8 : 0` (the bot holds lane centres and cannot weave, so chicanes rank last outright); pick the minimum (ties → nearest lane); steer toward the lane centre with a ±0.35 m deadband, otherwise toward the corridor centre. Gait 3 cruise, 2 within `braceAheadM` of any impulse hazard in its lane, 1 through rubble/scree. Brace within `braceAheadM` of a gap/rockfall/crane in its lane. Ratchet the loosest bay below `strapBelow` (select, then tap). Recover when an item is lost.
