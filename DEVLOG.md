...
- M1 fun gate (Task 17), headless metrics pass in place of human playtesting — 5/6 categories within target on the first constant change; full table in `.superpowers/sdd/2026-08-25-deadweight-core-loop/task-17-report.md`.
  - `driftThreshold`: 0.25 → 0.08. Metric e (drift-spiral onset: fore-slotted crate, `slopeRoute(0.35)`, ballast 0, gait 1) never crossed the old threshold at all — the fore crate's passive counterweight holds steady-state tilt at ≈0.1 rad, below 0.25, so drift (and the eventual spill) never started. Lowering the threshold below that steady tilt lets drift begin; item loss now follows ~18.5 s later (target ≥ 3 s — "a warning, not an ambush"). Verified this doesn't touch normal actively-controlled bot runs (bot.test.ts, validate.ts scenarios all stay at tilt ≈ 0, well under 0.08) — `pnpm test && pnpm validate` stayed green.
