...
- **Interceptor turret (PR #4)**: seeded emplacements sit off-corridor and fire a homing missile with a 5 s flight. The six danger levels are TIME TO IMPACT, not distance, and the shield can only be raised from a standstill — so braking time turns the encounter into an early commitment (at gait 4 you give up the run by level 2), not a reaction test. Aimed by pressing one of eight 45-degree sectors on a 360 threat scope; pointer only, no keybinds. Bearings never become angles: `src/sim` forbids `atan2`, so `octantOf` resolves them by sign and tangent comparisons. Measured tuning: cooldown 1800 / range 120 / shieldCost 3 gives 12/12 at lag 15 with a worst case of 10 reserve, against 19 for a bot that simply ate its missiles — worse in reserve, but every run now arrives and Lantern Reach at lag 30 went from spilled to 5 stars.
- **Temporal sandstorm (PR #3)**: seeded fronts scheduled in TICKS not metres, so driving slowly costs you more storm and sprinting can outrun the second front; 0-2 per route by tier, 5 s symmetric ramp with intensity derived from `t` so replay stays deterministic. Effects scale with the ramp — 30% speed cut, restraint drain, fog collapsing to ~15 m. RADAR (panel button, `V`) buys full draw distance back as a wireframe world for 0.5 reserve/s, is not storm-gated and has no auto-release. The bot radars through every front, so `pnpm validate` prices the expensive branch: 12/12 at lag 15 with `reserveBudget` unchanged at 0.62, worst case Cinder Stair at 19 reserve. Total storm time per route is capped at 0.40 of the run (`storm.maxStormFrac`) — measured as the largest cap causing zero strandings across 600 procedural tier-3 seeds, against a no-storm control that strands 17/600 on its own.
