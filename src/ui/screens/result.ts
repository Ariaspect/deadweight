...
export function renderResult(el: HTMLElement, result: RunResult, items: ItemDef[], onNext: () => void, review = '', label = 'CONTINUE'): void {
  ...
  el.innerHTML = `
    <div class="screen result">
      ...
      <div class="cash">PAYOUT ${Math.round(result.payout)} + HAUL ${Math.round(result.bonus)} + SALVAGE ${Math.round(result.discoveryBonus)}${result.rushBonus > 0 ? ` + RUSH ${Math.round(result.rushBonus)}` : ''} = <b>${result.total}</b> SCRAP</div>
